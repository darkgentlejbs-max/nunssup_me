...
export const ScheduleCalendar: React.FC = () => {
  ...
  const [calendarView, setCalendarView] = useState<'daily' | 'weekly' | 'monthly' | 'list'>('weekly');
  const [listFilter, setListFilter] = useState<'upcoming' | 'past' | 'all'>('upcoming');
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isBlockModalOpen, setIsBlockModalOpen] = useState(false);
  const [selectedAppointment, setSelectedAppointment] = useState<Appointment | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [editApptServices, setEditApptServices] = useState<{ id: string; name: string; price: number | ''; saveToMenu: boolean; showDropdown: boolean }[]>([]);
  ...
  const [apptServices, setApptServices] = useState([
    { id: 'initial', name: '', price: '' as number | '', saveToMenu: true, showDropdown: false }
  ]);
  ...
  // Close direct appointment modal
  const handleCloseDirectModal = () => {
    setIsAddModalOpen(false);
    setEditingApptId(null);
    setNewCustName('');
    setNewCustPhone('');
    setApptServices([{ id: Date.now().toString(), name: '', price: '', saveToMenu: true, showDropdown: false }]);
    setNewNotes('');
  };
  ...
  const handleCreateDirectBooking = (e: React.FormEvent) => {
    ...
    const validServices = apptServices.filter(s => s.name.trim() !== '');
    if (validServices.length === 0) return;

    let combinedName = validServices.map(s => s.name.trim()).join(' + ');
    let combinedPrice = validServices.reduce((sum, s) => sum + (Number(s.price) || 0), 0);

    validServices.forEach(s => {
      const isExisting = services.some(xs => xs.name === s.name.trim());
      if (!isExisting && s.saveToMenu) {
        addService({
          category: '눈썹' as const,
          name: s.name.trim(),
          price: Number(s.price) || 0,
          durationMinutes: 60,
          description: '원장님 수동 추가 시술',
          recommendedFor: '모든 고객',
        });
      }
    });

    let finalServiceId = 'custom';
    // If exactly one service and it exists, use its real ID for cleaner data
    if (validServices.length === 1) {
       const match = services.find(xs => xs.name === validServices[0].name.trim());
       if (match) finalServiceId = match.id;
    }
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-stone-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end w-full md:w-auto">
          ...
          <button
            ...
            className="px-3 py-2 bg-[#FAF5EE] hover:bg-[#F2EBD9] text-[#4A3728] border border-[#E8DBCA] rounded-xl text-xs font-bold transition-all flex items-center gap-1"
          >
            <Ban className="w-3.5 h-3.5 text-[#8E6C51]" />
            <span>일정 차단</span>
          </button>
        ...
        <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
            {getWeekDates(new Date(selectedDate)).map((wDate) => {
              ...
              const wApts = appointments.filter((a) => a.date === wDate);
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden border border-stone-200">
            <div className="bg-brand-900 text-white p-5 flex items-center justify-between shrink-0">
              ...
              <button
                onClick={handleCloseDirectModal}
            ...
            <form onSubmit={handleCreateDirectBooking} className="p-6 space-y-4 overflow-y-auto">
              <div className="relative">
                ...
                {showCustomerDropdown && (
                  <ul className="absolute z-10 w-full mt-1 max-h-48 overflow-y-auto bg-white border border-stone-200 rounded-xl shadow-lg">
                    {customers.filter(c => c.name.includes(newCustName)).length > 0 ? (
                      customers.filter(c => c.name.includes(newCustName)).map((customer) => (
                        <li
                          key={customer.id}
                          onMouseDown={(e) => {
                          ...
                          className="px-4 py-2 text-sm text-stone-700 hover:bg-stone-100 cursor-pointer flex justify-between items-center"
                        >
                      ...
                      <li className="px-4 py-3 text-xs text-stone-500 text-center">검색 결과가 없습니다.</li>
                    )}
                  </ul>
                )}
              </div>
              ...
              <div className="space-y-4">
                {apptServices.map((srv, index) => (
                  <div key={srv.id} className="relative bg-stone-50 p-3 rounded-xl border border-stone-200">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {apptServices.length > 1 && (
                        <button
                          type="button"
                          onClick={() => setApptServices(apptServices.filter(s => s.id !== srv.id))}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={srv.name}
                          onFocus={() => {
                            const newArr = [...apptServices];
                            newArr[index].showDropdown = true;
                            setApptServices(newArr);
                          }}
                          onBlur={() => setTimeout(() => {
                            setApptServices(prev => {
                              const newArr = [...prev];
                              if (newArr[index]) newArr[index].showDropdown = false;
                              return newArr;
                            });
                          }, 200)}
                          onChange={(e) => {
                          ...
                          className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-800"
                        />
                        {srv.showDropdown && (
                          <ul className="absolute z-10 w-[200%] max-w-sm mt-1 max-h-48 overflow-y-auto bg-white border border-stone-200 rounded-xl shadow-lg">
                            {services.filter(s => s.name.includes(srv.name) || s.category.includes(srv.name)).length > 0 ? (
                              services.filter(s => s.name.includes(srv.name) || s.category.includes(srv.name)).map((service) => (
                                <li
                                  key={service.id}
                                  onMouseDown={(e) => {
                                    e.preventDefault();
                                    const newArr = [...apptServices];
                                    newArr[index].name = service.name;
                                    newArr[index].price = service.price;
                                    newArr[index].showDropdown = false;
                                    setApptServices(newArr);
                                  }}
                                  className="px-4 py-2 text-sm text-stone-700 hover:bg-stone-100 cursor-pointer flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1"
                                >
                                  <span className="font-bold">[{service.category}] {service.name}</span>
                                  <span className="text-xs text-brand-700 font-mono font-bold">{formatCurrency(service.price)}</span>
                                </li>
                              ))
                            ) : (
                              <li className="px-4 py-3 text-xs text-stone-500 text-center">직접 입력 (신규 시술)</li>
                            )}
                          </ul>
                        )}
                      </div>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-stone-700 mb-1">
                          결제 금액 (원) <span className="text-rose-500">*</span>
                        </label>
                        <input
                          type="number"
                          required
                          ...
                          onChange={(e) => {
                            const newArr = [...apptServices];
                            newArr[index].price = e.target.value === '' ? '' : Number(e.target.value);
                            setApptServices(newArr);
                          }}
                          className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-brand-800"
                        />
                        {!services.find(s => s.name === srv.name) && srv.name.length > 0 && (
                          <div className="mt-2 flex items-center gap-2">
                            <input
                              type="checkbox"
                              id={`saveToMenu-${srv.id}`}
                              checked={srv.saveToMenu}
                              onChange={(e) => {
                                const newArr = [...apptServices];
                                newArr[index].saveToMenu = e.target.checked;
                                setApptServices(newArr);
                              }}
                              className="rounded text-brand-900 focus:ring-brand-900 border-stone-300"
                            />
                            <label htmlFor={`saveToMenu-${srv.id}`} className="text-xs text-stone-600 font-bold cursor-pointer">
                              메뉴 영구 추가
                            </label>
                          </div>
                        )}
                ...
                <button
                  type="button"
                  onClick={() => setApptServices([...apptServices, { id: Date.now().toString(), name: '', price: '', saveToMenu: true, showDropdown: false }])}
                  className="w-full py-2 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-xl text-xs font-bold border border-dashed border-stone-300 flex items-center justify-center gap-2"
                >
                  <Plus className="w-4 h-4" />
                  시술 프로그램 추가
                </button>
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden border border-stone-200">
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              {!isEditing ? (
                <>
                  <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 space-y-2 text-xs">
                    <div className="flex justify-between items-center mb-4">
                      <span className="font-bold text-stone-900 text-sm">예약 정보</span>
                      <button 
                        onClick={() => {
                ...
                <div className="space-y-4">
                  <div className="relative">
                    <label className="block text-xs font-bold text-stone-700 mb-1">고객명</label>
                    <input
                      type="text"
                    ...
                    {showEditCustomerDropdown && (
                      <ul className="absolute z-10 w-full mt-1 max-h-48 overflow-y-auto bg-white border border-stone-200 rounded-xl shadow-lg">
                        {customers.filter(c => c.name.includes(selectedAppointment.customerName)).length > 0 ? (
                          customers.filter(c => c.name.includes(selectedAppointment.customerName)).map((customer) => (
                            <li
                              key={customer.id}
                              onMouseDown={(e) => {
                                e.preventDefault();
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={selectedAppointment.customerPhone}
                      onChange={(e) => setSelectedAppointment({ ...selectedAppointment, customerPhone: e.target.value })}
                      className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-stone-700 mb-1">시간</label>
                      <input
                        type="time"
                  ...
                  <div className="space-y-4 border-y border-stone-100 py-4">
                    <label className="block text-xs font-bold text-stone-700">시술 프로그램</label>
                    {editApptServices.map((srv, index) => (
                      <div key={srv.id} className="relative bg-stone-50 p-3 rounded-xl border border-stone-200">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-xs font-bold text-stone-700">
                            시술 프로그램 {index + 1}
                          </span>
                          {editApptServices.length > 1 && (
                            <button
                              type="button"
                              onClick={() => setEditApptServices(editApptServices.filter(s => s.id !== srv.id))}
                              className="text-rose-500 hover:text-rose-700 text-xs font-bold"
                            >
                              삭제
                            </button>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="relative">
                            <label className="block text-[10px] font-bold text-stone-500 mb-1">시술명 <span className="text-rose-500">*</span></label>
                            <input
                              type="text"
                              required
                              placeholder="검색 또는 직접 입력"
                              value={srv.name}
                              onFocus={() => {
                                const newArr = [...editApptServices];
                                newArr[index].showDropdown = true;
                                setEditApptServices(newArr);
                              }}
                              onBlur={() => setTimeout(() => {
                                setEditApptServices(prev => {
                                  const newArr = [...prev];
                                  if (newArr[index]) newArr[index].showDropdown = false;
                                  return newArr;
                                });
                          ...
                          <div>
                            <label className="block text-[10px] font-bold text-stone-500 mb-1">결제 금액 (원) <span className="text-rose-500">*</span></label>
                            <input
                              type="number"
                              required
                              placeholder="0"
                            ...
                            {!services.find(s => s.name === srv.name) && srv.name.length > 0 && (
                              <div className="mt-2 flex items-center gap-2">
                                <input
                                  ...
                                  className="rounded text-brand-900 focus:ring-brand-900 border-stone-300"
                                />
                                <label htmlFor={`saveToMenuEdit-${srv.id}`} className="text-[10px] text-stone-600 font-bold cursor-pointer">
                                  메뉴 추가
                                </label>
                              </div>
                            )}
                    ...
                    <button
                      type="button"
                      onClick={() => setEditApptServices([...editApptServices, { id: `edit-${Date.now()}`, name: '', price: '', saveToMenu: true, showDropdown: false }])}
                      className="w-full py-2 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-xl text-xs font-bold border border-dashed border-stone-300 flex items-center justify-center gap-2"
                    >
                      <Plus className="w-4 h-4" />
                      시술 추가
                  ...
                  <div>
                    <label className="block text-xs font-bold text-stone-700 mb-1">메모</label>
                    <textarea
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <button
                      onClick={() => {
                        const validServices = editApptServices.filter(s => s.name.trim() !== '');
                        if (validServices.length === 0) {
                          alert('최소 1개의 시술을 입력해주세요.');
                          return;
                        }
                      ...
                      className="flex-1 py-2 bg-brand-900 hover:bg-brand-800 text-white rounded-xl text-xs font-bold transition-colors"
                    >
                      저장하기
                    </button>
                  </div>
