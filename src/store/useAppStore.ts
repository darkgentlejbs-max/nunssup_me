...
export const useAppStore = create<AppStore>((set, get) => {
  ...
  return {
    ...
    createAppointment: (data) => {
      ...
      let finalDuration = 60; // default for custom

      if (data.serviceId === 'custom') {
        finalServiceId = `custom-${Date.now()}`;
        finalServiceName = data.customServiceName || '직접 입력 시술';
        finalServicePrice = data.customServicePrice || 0;
      } else {
        ...
        finalDuration = service.durationMinutes;
      }
      ...
      get().showToast(
        '예약 신청 완료',
        `${data.customerName}님의 [${finalServiceName}] 예약이 접수되었습니다. (100% 예약제 확인 후 안내)`,
        'success'
      );
